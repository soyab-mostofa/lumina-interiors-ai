    ...
    <div className={`fixed bottom-6 left-6 z-50 w-full max-w-[360px] md:max-w-[400px] flex flex-col shadow-2xl transition-all duration-300 ${isMinimized ? 'animate-slide-down' : 'animate-slide-up'}`}>
      ...
      <div className="bg-slate-900 text-white p-4 rounded-t-2xl flex justify-between items-center border-b border-slate-800">
        ...
        <div className="flex items-center gap-1">
          ...
          <button onClick={onClose} className="p-1 hover:bg-slate-800 rounded-full transition-colors" title="Close">
            <X size={18} />
          </button>
        </div>
      </div>
        ...
        <div className="flex-grow bg-gradient-to-b from-white to-slate-50/50 backdrop-blur-xl h-[450px] overflow-y-auto p-4 space-y-4 border-x border-slate-200 custom-scrollbar">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    {quickActions.map((action, idx) => (
                      <button
                        ...
                        className="bg-white border border-slate-200 hover:border-indigo-300 hover:bg-indigo-50 rounded-xl p-3 text-left transition-all group"
                      >
                        <div className="text-2xl mb-1">{action.icon}</div>
                        <div className="text-xs font-semibold text-slate-700 group-hover:text-indigo-700">{action.text}</div>
                      </button>
                    ))}
                  </div>
                </div>
          ...
          {messages.map((msg) => (
            <div key={msg.id} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'} animate-fade-in`}>
               {msg.isSystemMessage ? (
                  <div className="flex items-center gap-2 text-xs font-bold text-indigo-600 bg-indigo-50 px-4 py-2 rounded-full mx-auto shadow-sm animate-pulse">
      ...
      {!isMinimized && (
        <div className="bg-white p-3 border-t border-slate-200 rounded-b-2xl">
          <div className="flex items-end gap-2 bg-slate-50 border border-slate-200 rounded-xl px-3 py-2 focus-within:ring-2 focus-within:ring-indigo-500 focus-within:border-indigo-300 transition-all">
            <textarea
              ref={inputRef}
              value={input}
              onChange={(e) => {
            ...
            <div className="flex items-center gap-2 flex-shrink-0">
              {input.length > 0 && (
                <span className={`text-xs font-medium transition-colors ${isNearLimit ? 'text-amber-600' : 'text-slate-400'}`}>
                  {remainingChars}
                </span>
              )}
              <button
                ...
                className="p-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 disabled:opacity-50 disabled:cursor-not-allowed transition-all hover:scale-105 active:scale-95"
